...
def run_transaction(mode_name):
    ...
    print(f"\n🔁 {Colors.UNDERLINE}Initial State:{Colors.ENDC} {Colors.OKCYAN}{current_state}{Colors.ENDC}")

    while current_amount < price:
        remaining = price - current_amount
        print(f"\n💰 {Colors.BOLD}Remaining:{Colors.ENDC} RM{remaining:.2f}")
        print(f"📍 Current State: {Colors.OKCYAN}{current_state}{Colors.ENDC}")
        try:
        ...
        if note not in valid_notes:
            if mode_name == "DFA":
                ...
                print("💥 Transaction aborted. Goodbye.")
...
def vending_machine():
    ...
    if mode_name == "DFA":
        ...
        print(f"\n{Colors.FAIL}Thank you for using the DFA Vending Machine. Goodbye!.{Colors.ENDC}")
        return

    while True:
        run_transaction(mode_name)
        again = get_valid_input("\n🔁 Do you want to make another purchase? (Y/N): ", ['Y', 'N'])
        if again == "no":
